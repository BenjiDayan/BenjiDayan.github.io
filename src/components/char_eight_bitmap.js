...
function printChar(char) {
    ...
    for(var row = 0; row < bmp.length; row++) {
        ...
        for(var col = 0; col < t.length; col++) {
            out_arr[out_arr.length - 1].push(parseInt(t[col], 2));
        }
    }
    return out_arr;
}
...
function printChars(str) {
    ...
    for (const char of str) {
        var char_arr = printChar(char);
        var i;
        for (i=0; i < char_arr.length; i++) {
            if (i < out_arr.length) {
                out_arr[i].push.apply(out_arr[i], char_arr[i]);
            } else {
                out_arr.push(char_arr[i]);
            }
        }
